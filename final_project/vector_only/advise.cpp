...
void ALL::order_function(const int string_length,const int& length_dis,const int& SCORE,const int ID_length,vector<int>& record_array,int string_score_remain,int flag){
	if(string_score_remain==0){
		...
		if(record_array.size()==0){
			if(findVector(advise_string)<=0&& strcmp(advise_string," ")!=0){
				char *tmp = new char [101];
				strcpy(tmp, advise_string);
				TEN.push_back(tmp);
			}
	...
	if(string_score_remain>=(string_length-flag)){
		...
		for(flag;flag<string_length;flag++){
			...
			order_function(string_length,length_dis,SCORE,ID_length,record_array,string_score_remain-(string_length-flag),flag+1);
			record_array.pop_back();
		}
	}
	else{
		//cout<<"enter else"<<endl;
		order_function(string_length,length_dis,SCORE,ID_length,record_array,string_score_remain,flag+1);
	}
	//================================
	return;
}
void ALL::order_function_big(const int string_length,const int& length_dis,const int& SCORE,const int ID_length,vector<int>& record_array,int string_score_remain,int flag){
	if(string_score_remain==0){	
		char advise_string[101];
	...
	if(string_score_remain>=(ID_length-flag)){
		for(flag;flag<ID_length;flag++){
			record_array.push_back(flag);
			order_function_big(string_length,length_dis,SCORE,ID_length,record_array,string_score_remain-(ID_length-flag),flag+1);
			record_array.pop_back();
		}
	}
	else{
		order_function_big(string_length,length_dis,SCORE,ID_length,record_array,string_score_remain,flag+1);
	}
	//================================
		...
		//j is the distance between strings
			...
			if(j>=0){
				...
				if(strlen(used_ID)>j){
					...
					order_function(strlen(used_ID)-j,j,SCORE,strlen(used_ID),record_array,string_score_remain,0);}
			}
			else{
				string_score_remain=SCORE-(j*(-1)*(j*(-1)+1))/2;//string_score_remain is the score which minus length
				if(strlen(used_ID)-j<=100){
					order_function_big(strlen(used_ID)-j,(-1)*j,SCORE,strlen(used_ID),record_array,string_score_remain,0);
				}
			}
		//cout<<record<<endl;
		}
