...
void Div(vector<DATA> &data, int max_index, double std_confusion, int depth)
{
	...
	double div_point; int div_cond, div_pos, yn = 0;
	int size = data.size();
	int allsum_Y = 0;
	...
	if (Confusion(allsum_Y, size-allsum_Y) <= std_confusion+0.000001 || fabs(min_confusion - 1) < 0.000001) {
		PrintTab(depth);printf("%c++;\n", (allsum_Y >= size-allsum_Y)? 'T' : 'F');
		return;
	}
...
int main(int argc, char const *argv[])
{
	...
	for (int i = 1 ; i <= T ; i++) {
		...
		for (int j = 0 ; j < data.size()/3 ; j++) {
			shuffle.push_back(data[rand()%data.size()]);
		}
		Div(shuffle, max_index, 0, 1);
	}
	printf("voting:\n\tif(T >= F) return 1;\n\telse return 0;\n");
	printf("}\n");
	return 0;
